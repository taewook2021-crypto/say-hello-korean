...
-- Drop constraints with CASCADE
ALTER TABLE public.subjects DROP CONSTRAINT subjects_name_key CASCADE;
ALTER TABLE public.books DROP CONSTRAINT books_name_subject_name_key CASCADE;

-- Add new unique constraints that include user_id
ALTER TABLE public.subjects ADD CONSTRAINT subjects_name_user_id_key UNIQUE (name, user_id);
ALTER TABLE public.books ADD CONSTRAINT books_name_subject_name_user_id_key UNIQUE (name, subject_name, user_id);

-- Update RLS policies
DROP POLICY IF EXISTS "Allow anonymous access for development" ON public.subjects;
CREATE POLICY "Users can manage their own subjects" ON public.subjects FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Allow anonymous access for development" ON public.books;
CREATE POLICY "Users can manage their own books" ON public.books FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Allow anonymous access for development" ON public.chapters;
CREATE POLICY "Users can manage their own chapters" ON public.chapters FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Allow all operations for development" ON public.wrong_notes;
CREATE POLICY "Users can manage their own wrong_notes" ON public.wrong_notes FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);