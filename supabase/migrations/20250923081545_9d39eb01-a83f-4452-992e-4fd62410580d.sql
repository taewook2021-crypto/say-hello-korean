-- Fix existing data and implement user isolation with proper foreign key handling

-- 1. Delete NULL user_id records first
...
-- 2. Make user_id NOT NULL for all tables
...
-- 3. Drop foreign key constraints that depend on the unique constraint
...
-- 4. Drop and recreate unique constraints with user_id
ALTER TABLE public.subjects 
...
-- 5. Update RLS policies for proper user isolation
